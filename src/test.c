...
// WARNING	Queste funzioni sono qui solo per facilitare il debug. In futuro dovranno 
//			essere spostate in una libreria condivisa
...
typedef uint16_t bitmap_byte_len_t;
...
{
	if (enable)
		*((uint8_t*)buffer + offset/8) |= (1 << (7 - offset%8));
	else
		*((uint8_t*)buffer + offset/8) &= ~(1 << (7 - offset%8));
}
